...
- `/api/sync/media` rejects stale preview ids and unknown operations before provider access; provider unit tests cover Apple catalog previews, QQ detail-driven `file.media_mid` vkey playback, NetEase v1 fallback, real artwork extraction, and unavailable-media degradation. Pure alignment tests cover exact offsets, unrelated-audio rejection, and insufficient-overlap fallback.
...
- `/api/auto-sync` starts disabled, rejects enablement before readiness, persists protected settings after readiness, and exposes sanitized history. `/api/auto-sync/run` defaults to dry-run, records attention history without provider mutation, and never executes deletion signals.
...
- `/api/sync/identity-decision` is exercised through keep, separate, and clear: pure equivalence tests prove the incremental operation patch has the same action shape as a full canonical rebuild, the separate response identifies additions that need target candidate search, and no provider write occurs.
...
- Seven-screen navigation: overview, connect platforms, sync mode, sync preview, automatic sync, AI assistant, and advanced settings.
...
- The React default `/` replacement path is checked on desktop and mobile with real fixture data, automatic-sync readiness / history, sync preview generation, controlled write guards, local AI actions, Agent audit refresh, advanced diagnostics, and no horizontal overflow.
- React browser smoke opens the manual-review bucket, verifies source / target version rows, resolved artwork, one paused player, no demo covers, source-relative aligned A/B switching with preserved elapsed position, preview pagination, and zero desktop / mobile horizontal overflow.
...
This starts the local web server on an OS-assigned localhost port against an isolated temporary `MUSIC_LIKES_SYNC_HOME`. It seeds Apple and target fixture snapshots, generates an Apple-source-of-truth mirror plan, checks `/api/state`, `/api/app/state`, `/api/sync/modes`, `/api/ai/provider`, `/metrics`, static-file path traversal / bad-encoding guards, invalid / oversized JSON request guards, validates ordinary-user sync check / preview, consent guards for addition and identity AI review, durable keep / undo identity decisions across full preview regeneration, product add resolution through `/api/sync/resolve-additions`, product canonical multi-target add-only execution, real product execution blocking when target live-validation evidence is missing, managed ready-add and confirmed-delete dry-runs, sanitized `sync-runs`, post-write convergence summaries, deterministic / consent-gated music intelligence, baseline diff examples, single and batch tombstone decisions, delete-only separation, mirror review batches, add-only / remove-only idempotency, blocked QQ mid-only removal, and mirror convergence rebuilds.

It explicitly verifies that deletion execution fails before `POST /api/sync/confirm-deletions` and does not write into the checkout's local `data/` or `reports/`.
