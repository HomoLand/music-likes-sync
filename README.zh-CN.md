...
- 匹配层统一处理 Unicode、完整简繁转换、平台 / MusicBrainz 别名、日文假名与罗马音变体；同一艺人的已验证别名会跨歌曲复用，同一套变体同时用于确定性评分、AI 证据和安全门。
- 当标准化标题与专辑精确一致、时长相差不超过 2 秒、无版本标记冲突且不存在不同 ISRC 时，系统会把组合证据视为“录音指纹”自动保留；多个指纹候选仍进入复核，AI 只判断剩余歧义，不能越过写入或删除保护。
